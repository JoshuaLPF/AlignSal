...
class DynamicFilter(nn.Module):
    ...
    def forward(self, x):
        B, H, W, _ = x.shape #1,24,24,320

        routeing = self.reweight(x.mean(dim=(1, 2))).view(B, self.num_filters,
                                                          -1).softmax(dim=1) # 1,4,640
        # routeing = self.reweight(x.mean(dim=(1, 2))) # 1,2560
        # routeing = routeing.view(B, self.num_filters, -1).softmax(dim=1) # 1,4,640

        x = self.pwconv1(x) # 1,24,24,640
        x = self.act1(x)
        x = x.to(torch.float32)
        x = torch.fft.rfft2(x, dim=(1, 2), norm='ortho') # 1,24,13,640
        ...
        if self.weight_resize:
            ...
            complex_weights = torch.view_as_complex(self.complex_weights)  # 24,13,4
        routeing = routeing.to(torch.complex64) # 1,4,640
...
class DynamicFilterConv(nn.Module):
    def __init__(self, dim, act1_layer=StarReLU, act2_layer=nn.Identity, num_filters=4, size=14, weight_resize=False,
                 **kwargs):
        super().__init__()
        size = to_2tuple(size)
        self.size = size[0]
        ...
        self.pwconv1 = nn.Conv2d(dim, self.med_channels, kernel_size=1, stride=1, padding=0, bias=False)
        self.max_pool = nn.AdaptiveMaxPool2d(1)

    def forward(self, x):
        B, H, W, _ = x.shape #1,24,24,320
        shortcut = x
        x_pre = self.pwconv1(x.permute(0, 3, 1, 2))  # 1,24,24,640
        routeing = self.max_pool(x_pre).permute(0, 2, 3, 1).view(B, self.num_filters, -1).softmax(dim=1) # 1,4,320
        x = self.act1(x)
        x = x.to(torch.float32)
        x = torch.fft.rfft2(x, dim=(1, 2), norm='ortho')
        if self.weight_resize:
        ...
        weight = torch.einsum('bfc,hwf->bhwc', routeing, complex_weights)  # 1,24,13,320
        x = x * weight
        x = torch.fft.irfft2(x, s=(H, W), dim=(1, 2), norm='ortho')
...
class Mlp(nn.Module):
    ...
    def __init__(self, dim, mlp_ratio=4, out_features=None, act_layer=StarReLU, drop=0.,
        ...
        self.fc1 = nn.Linear(in_features, hidden_features, bias=bias)  # rate =0.25   in_features 320  out_features 80
        self.act = act_layer()
        self.drop1 = nn.Dropout(drop_probs[0])
        self.fc2 = nn.Linear(hidden_features, out_features, bias=bias) # in_features 80  out_features 2560 = 4(num_filters) * 2 * 320 (med_channels = expansion_ratio * dim)
...
}    #  C:\Users\Joshua\.cache\torch\hub\checkpoints
...
def dfformer_s18(pretrained=False, **kwargs):
    ...
    model = MetaFormer(
        ...
        # token_mixers=DynamicFilterConv,
        head_fn=MlpHead,
...
def cdfformer_s18(pretrained=True, **kwargs):
    ...
    model = MetaFormer(
        ...
        # token_mixers=[SepConv, SepConv, DynamicFilterConv, DynamicFilterConv],
        head_fn=MlpHead,
        input_size=default_cfg['input_size'],
        **kwargs)
    model.default_cfg = default_cfg
